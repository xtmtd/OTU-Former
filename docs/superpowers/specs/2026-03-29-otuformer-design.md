...
- `--orientation-policy` : orientation policy — `invariant` (new-run default) or `sensitive`; when initializing from `--checkpoint`, an omitted value inherits the pretraining checkpoint's saved policy
